...
public enum ErrorCode {
    ...
    MEMBER_ACADEMY_INVALID("로그인한 멤버는 학원에 소속되어 있지 않습니다.", HttpStatus.UNPROCESSABLE_ENTITY),

    EMAIL_NOT_FOUND("계정이 존재하지 않습니다.", HttpStatus.NOT_FOUND),
    EMAIL_ALREADY_EXISTS("이미 가입된 이메일입니다.", HttpStatus.CONFLICT),
    EMAIL_TYPE_INVALID("지원하지 않는 이메일입니다.", HttpStatus.BAD_REQUEST),
    EMAIL_AUTH_CODE_INVALID_OR_EXPIRED("인증 코드가 만료되었거나 존재하지 않습니다.", HttpStatus.NOT_FOUND),
    EMAIL_AUTH_CODE_INVALID("유효하지 않은 코드입니다.", HttpStatus.NOT_FOUND),
    ...
    ACADEMY_ACCESS_INVALID("해당 학원에 접근할 권한이 없습니다.", HttpStatus.FORBIDDEN),
    ...
    ACADEMY_INVITATION_INVALID("학원으로부터 초대받지 않은 계정입니다.", HttpStatus.BAD_REQUEST),
    ...
    MAIN_CLASS_ACCESS_INVALID("사용자가 속한 학원에 존재하는 메인 클래스가 아닙니다.", HttpStatus.FORBIDDEN),
    ...
    ATTENDANCE_STATUS_INVALID("유효하지 않은 출결 타입입니다.", HttpStatus.BAD_REQUEST),
    ATTENDANCE_WEEK_VALUE_INVALID("4주 전부터 2주 후까지 조회가 가능합니다.", HttpStatus.BAD_REQUEST),
