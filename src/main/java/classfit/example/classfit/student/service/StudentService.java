...
public class StudentService {
    ...
    private void createAttendanceForSevenWeeks(Student student) {
        ...
        enrollments.forEach(classStudent -> {
            for (int i = 0; i < 7; i++) {
                ...
                AttendanceStatus status = (i < 4) ? AttendanceStatus.ABSENT
                        : AttendanceStatus.PRESENT; // 4주 전은 ABSENT, 나머지는 PRESENT
    ...
    public void deleteStudent(Member member, List<Long> studentIds) {
        studentIds.stream()
                ...
                .forEach(student -> {
                    List<ScoreReport> scoreReports = scoreReportService.findByStudentId(
                            student.getId());
